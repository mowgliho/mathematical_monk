...
- An example of a "prefix code" is (A) - we can decode upon finishing the letter: don't have to look farther forward
...
- Let
  ...
  - What is the expected encoded length $L^1 = \sum l(x)p^1(x)$ for $x \in \mathcal{X}^1$
    - $l(x)$ is the number of bits to encode for $[a,b)$ for $x_1\dots x_k0$, i.e. $\min\{m: \exists \beta_1,\dots,\beta_m\in\{0,1\}$ s.t. $[0.\beta_1\dots\beta_m,0.\beta_1\dots\beta_m + 1/2^m)\subset [a,b)\}$
...
- Quite simple: just use the non-iid probabilities instead.
  - previously: $p^1(x_1\dots x_k0) = p_{x_1}p_{x_2}\dots p_{x_k}0$, $\mathcal{X} = \{0,1,\dots,n\}$, $p = (p_0,p_1,\dots, p_n)$, $\mathcal{X}^1 = \{x_1\dots x_k0: x_i\in \mathcal{X}\setminus\{0\}, k \geq 0\}$
  - as $p$ is not iid in practice, we are pretty inefficient in our compression
  - We can factor:
    - $q(x_1\dots x_k0)=q(x_1)q(x_2|x_1)q(x_3|x_1,x_2)\dots q(0|x_1,\dots,x_k)$
    - note that different elements of $\mathcal{X}^1$ have different lengths $k+1$, but this is still ok.
  - we were able to get close to the ideal entropy because $b-a = p^1(x)$
    - because each time we focused in, we looked at length equal to $p_i$
    - therefore, this time we focus in on $q_i(x_i|x_1,\dots,x_{i+1})$
      - i.e. subdivide based on the conditional probabilities
  - we have to be able to compute $q$ efficiently though
- Decoding: we do the same thing as before (subdivide by $q_i$ instead of $p$, and it works out)
- EOF symbol:
  - if we predict when the end of file symbol is coming, then get better compression than if we had just sent the length of the message
    - i.e. we can use the content of the message to predict when the EOF comes
- Example in practice: $m$-th order Markov model

# 5.11: Finite-precision arithmetic coding - Rescaling

- What went wrong in the infinite precision algorithm
  - $b-a$ becomes very small
    - so we rescale things periodically
      - before, we had three options:
        - lower half and emit 0
        - upper half and emit 1
        - straddle middle and keep track
        - note that if we straddle middle but are outside of quartiles, we can easily find an interval in $[a,b)$ and we're done.
      - now, we'll integrate the rescaling in the calculation of $a$ and $b$
        - before, we calculate $a$ and $b$ first, and then find the interval
        - now we combine
          - rescaling and subdividing commutes
          - if both $a$ and $b$ are close to 0.5, then we zoom in until either $a<1/4$ or $b>3/4$ - and emit $011\dots$ or $100\dots$ as necessary ($s$ counter)and continue loop
        - algorithm is now as follows:
          ...
          - for $i=1,\dots,k+1$:
            ...
            - $a := a + wc_{x_i}$
            - while $b < 1/2$ or $a>1/2$:
              - if $b<1/2$ emit 0 and $s$ $1$s, $a:= 2a$, $b:= 2b$, $s:=0$
              - else emit 1 and $s$ $0$s, $a:= 2(a-1/2)$, $b:= 2(b-1/2)$, $s:=0$
            - while $a>1/4$ and $b<3/4$:
              - $s:= s+1$, $a:= 2(a-1/4)$, $b:= 2(b-1/4)$
- making the decoder rescaling sync up with the encoder rescaling will be a bit tough.

# 5.12: Finite-precision arithmetic coding - Setup

- Define 
  - precision to be the number of bits to use to represent numbers (e.g. 32)
  - $W=2^{precision}$, $H = W/2$, $Q = W/4$.
  - let $\mathcal{X} = \{0,1,\dots,n\}$, EOF is 0.
  - let $p = (p_0,p_1,\dots,p_n)$ be a pmf on $\mathcal{X}$ s.t. $p_i = r_i/R$, where $r_i\in\mathbb{Z}$ and $R=\sum_0^nr_i$
    - in practice we may have to round off the $p_i$ to get them as rationals.
    - we have to be able to present $RW$ whole exactly (be aware of limitations of programming language).
  - Let $x_1,\dots,x_k\in\mathcal{X}\setminus\{0\}$, $x_{k+1}=0$
  - $c_0 = 0$, $c_j = r_0 +  \dots + r_{j-1}$ for $j=1,\dots,n$
  - $d_j = c_j + r_j$ for $j = 0,\dots, n$.
  - i.e. the $c$ and $d$ are scaled up by $R$

# 5.13: Finite-precision arithmetic coding - Encoder

- we've done most of the work already, now we just work with integers
- given the setup from the last one
  - $\text{round}(x)$ is a language specific operation that chooses an integer closeto $x$
    - e.g. ceil/floor, division with remainder (thus we're keeping things in integers)
- Algorithm
  - $a:= 0$, $b:= W$, $s: = 0$
  - for $i=1,\dots, k+1$:
    - $w:= b-a$
    - $b = a + \text{round}(wd_{x_i}/R)$
    - $a:= a + \text{round}(wc_{x_i}/R)$
    - while $b < H$ or $a > H$:
      - if $b<H$ emit 0 and $s$ 1s, $s:= 0$, $a:= 2a$, $b:= 2b$
      - else: emit $1$ and $s$ 0s, $s:=0$, $a := 2(a-H)$, $b = 2(b-H)$
    - while $a > Q$ and $b<3Q$:
      - $s:= s+1$, $a:= 2(a-Q)$, $b = 2(b-Q)$
  - $s:= s+1$
  - if $a\leq Q$ emit $0$ and $s$ 1s
  - else emit $1$ and $s$ 0s
  - output is the binary sequence emitted
- i.e. the same as our rescaled model, with a minor modification to do integers
  - note that the $a$ and $b$ are different because of the rounding off
  - we'll have to account for this in the decoding

# 5.14: Finite-precision arithmetic coding - Decoder

- Input: $\beta_1\dots \beta_m\beta_{m+1}\dots\beta_M$
- we have to rescale at exactly the same time as in the encoder
- Algorithm
  - $a:= 0$, $b:= W$, $z:= 0$, $i = 1$
  - while $i\leq precision$ and $i\leq M$:
    - if $\beta_i =1$, then $z = z+2^{precision - i}$, $i:= i+1$
    - (i.e. we are doing $z = 0.\beta_1\dots\beta_{precision}$ in binary)
  - while True:
    - for $j=0,1,\dots,n$:
      - $w := b-a$
      - match up rounding algorithms
        - $b_0 = a + \text{round}(wd_{x_j}/R)$
        - $a_0 = a + \text{round}(wc_{x_j}/R)$
      - test the $j$
        - if $a_0 \leq z < b_0$
          - emit $j$, $a:= a_0$, $b:= b_0$
          - if $j==0$, quit (EOF)
      - i.e. decode next symbol and update $a,b$
    - while $b < H$ or $a > H$
      - rescaling, synced with encoding, i.e. "left shift" the binary representation
      - if $b < H$, then $a:= 2a$, $b:= 2b$, $z:= 2z$
      - else $a:= 2(a-H)$, $b:= 2(b-H)$, $z:= 2(z-H)$
      - if $i \leq M$ and $\beta_i = 1$, then $z:= z+1$ (remember that we're in binary)
      - $i:= i+1$
    - while $a > Q$ and $b < 3Q$:
      - rescaling the "straddle case"
      - $a := 2(a-Q)$, $b := 2(b-Q)$, $z := 2(z-Q)$
      - if $i\leq M$ and $\beta_i = 1$, then $z:= z+1$
      - $i := i+1$
  - output is the sequence emitted
  - determining $m$ is a bit tricky, but can be done
