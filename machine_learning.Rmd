...
- Forward Algorithm:
  ...
  - $p(z_k,x_{1:k}) = \sum_{z_{k-1}}^m p(z_k,z_{k-1},x_{1:k}) = \sum_{z_{k-1}} p(x_k|z_k,z_{k-1},x_{1:k-1})p(z_k|z_{k-1},x_{1:k-1})p(z_{k-1}|z_{k-1},x_{1:k-1})$
    ...
    - Denote $\alpha_k(z_k) := p(z_k,x_{1:k})$
      ...
      - for $k=1$, $\alpha_1(z_1) = p(z_1,x_1) = p(z_1)p(x_1|z_1)$, i.e. product of initial and emission probabilities
  ...
  - consider the naïve approach 
    - $p(z_k|x) = p(z_k,x)/p(x)$
      - $p(z_k,x) = \sum_{z_i, i\neq k} p(x,z)$
        - so have to calculate $p(x,z)$ approximately $m^n$ times - i.e. all possible sequences of $z$
...
- Backard Algorithm
  - Goal: Compute $p(x_{k+1:n}|z_k)$ for all $k\in\{1,\dots,n-1\}$, $z_k\in\{1,\dots,m\}$
  - Let's set up a recursion like in the Forward Algorithm
  - $p(x_{k+1:n}|z_k) = \sum_{z_{k+1}=1}^m p(x_{k+1:n},z_{k+1}|z_k) = \sum_{z_{k+1}} p(x_{k+2:n}|z_{k+1},z_k, x_{k+1})p(x_{k+1}|z_{k+1},z_k)p(z_{k+1}|z_k)$
  - By $d$-separation/Markov
    - $p(x_{k+2:n}|z_{k+1},z_k,x_{k+1}) = p(x_{k+2:n}|z_{k+1})$
    - $p(x_{k+1}|z_{k+1},z_k) = p(x_{k+1}|z_{k+1})$
  - thus this becomes $\sum_{z_{k+1}} p(x_{k+2:n}|z_{k+1}) p(x_{k+1}|z_{k+1})p(z_{k+1}|z_k)$
    - Let $\beta_k(z_k):= p(x_{k+1:n}|z_k)$
      - this becomes $\beta_k(z_k) = \sum_{z_{k+1}} \beta_{k+1}(z_{k+1}) p(x_{k+1}|z_{k+1})p(z_{k+1}|z_k)$
      - valid for $k = 1,\dots,n-1$
        - for $k = n-1$, $\beta_n(z_n) = p(x_n|z_{n-1}) = \sum_{z_n} p(x_n|z_n)p(z_n|z_{n-1})$
          - Thus by analogy, $\beta_n(z_n)$ should be $1$ for all $z_n$
  - Thus calculate backwards from $n$ to $k$, same order of complexity as the forward algorihtm, etc.

## 14.10: Underflow and the log-sum-exp trick

- In practice, we will often run into underflow as probabilities get really small.
- log-sum-exp trick in HMMs
  - $\alpha_k(z_k) = p(z_k,x_{1:k}) = \sum_{z_{k-1}}p(x_k|z_k)p(z_k|z_{k-1})\alpha_{k-1}(z_{k-1})$
    - $\log \alpha_k(z_k) = \log\big(\sum_{z_{k-1}}\exp\big[\log p(x_k|z_k) + \log p(z_k|z_{k-1}) + \log\alpha_{k-1}(z_{k-1})\big]\big)$
      - this is not any better
      - let $a_i = \log p(x_k|z_k) + \log p(z_k|z_{k-1}) + \log\alpha_{k-1}(z_{k-1})$
        - typically, the $a_i$ are large negative numbers
      - then we want to compute $\log \sum_i e^{a_i}$
        - this is approximately $\max a_i$, so only the largest $a_i$ counts
        - so we consider $\log(e^b\sum_i(e^{a_i-b}))$, where $b = \max_i a_i$
        - this becomes $b + \log \sum_i e^{a_i-b}$
          - this doesn't have the underflow issue, as we essentially work in logs

## 14.11-14.12: Viterbi Algorithm

- find the most probable $z$ given $x$ and the initial/emission/transition probabilities
- Goal: Compute $\arg\max p(z|x)$
- Remark: If $f(a)\geq 0$ for all $a$ and $g(a,b)\geq 0$ for all $a,b$, then $\max_{a,b}f(a)g(a,b) = \max_a(f(a)\max_b(g(a,b)))$
- Note: $\arg\max_z p(z|x) = \arg\max_z p(z,x)$
- Viterbi Algorithm
  - Define $\mu_k(z_k) = \max_{z_{1:k-1}} p(z_{1:k},x_{1:k}) = \max_{z_{1:k-1}} p(x_k|z_k)p(z_k|z_{k-1})p(z_{1:k-1},x_{1:k-1})$
    - $\mu_k(z_k)$ defines the maximum probability for $z_{1:k}$, $x_{1:k}$, where $z_{1:k}$ ends in $z_k$ and we're getting the maximum over possible sequence $z_{1:k-1}$
    - use the remark, note that $p(x_k|z_k)$ and $p(z_k|z_{k-1})$ are independent of $z_{1:k-2}$, and obtain
      - $\mu_k(z_k) = \max_{z_{k-1}}p(x_k|z_k)p(z_k|z_{k-1})\mu_{k-1}(z_{k-1})$
  - Thus we have a nice recursion which holds for $k > 1$
  - consider $k=1$, i.e. $\mu_1(z_1) = p(z_1,x_1) = p(z_1)p(x_1|z_1)$
  - now we can calculate all the $\mu_k(z_k)$ iteratively
    - get $\mu_n(z_n)$.
    - $\max_{z_n}\mu_n(z_n) = \max_{z_{1:k}}p(x_{1:n},z_{1:n})$, i.e. our goal!
- to get the maximum sequence, we keep track of the $z_{1:k}$ sequence at each $\mu_{k}(z_k)$ that gives us the maximum

# 15: Newton's method for optimization and Logistic Regression

## 15.1-15.2: Newton's method (for optimization)

- Newton's Method: General Thoughts
  - Method for finding the zeros of a function
  - When applied to optimization, can find the maxima/minima of a function
  - Second order method (uses second derivative)
    - For example, Gradient Descent is a First order method
- Intuition
  - Zero-finding: $x_{t+1} = x_t - f(x_t)/f'(x_t)$, iterate and get zero
  - Minimizing/Maximizing
    - Usually, set derivative equal to zero
    - if can't solve analytically, can use Newton's Method $x_{t+1} = x_t - f'(x_t)/f''(x_t)$
      - i.e. find zero point of derivative
      - can also view as approximating the function with a quadratic at $x_t$ and setting $x_{t+1}$ to the minimum/maximum $x$ of this quadratic.
    - 2D: Can view as moving to minimum of paraboloid that approximates $f$ at $x_t$
- Derivation
  - Let $f:\mathbb{R}^n \rightarrow \mathbb{R}$ be "sufficiently smooth"
  - Taylor's theorem: given $a$, $f$ can be approximated by $f(x)\approx q(a) := f(a) + g^T(x-a) + 1/2 (x-a)^TH(x-a)$
    - Notes:
      - $x$ is close to $a$
      - $g = \nabla f(a)$
      - $H = \nabla^2 f(a)$
    - Expanding, get $\frac{1}{2}x^THx + b^Tx + c$ where
      - $b = g-Ha$
      - $c = f(a) + \dots$
    - Then $\nabla q = Hx + b$
      - This being 0 implies $x = - H^{-1}b = a - H^{-1}g$, assuming that $H$ is invertible
      - Let's check that this is a minimum
        - take another derivative, $\nabla^2 q = H$
          - i.e. the Hessian of $q$ is the Hessian of $f$
          - If $f$ is convex at $a$, then $q$ is convex at $a$
  - Algorithm:
    - initialize $x_0\in \mathbb{R}^n$
    - iterate $x_{t+1} = x_t - H^{-1}g$
      - $g = \nabla f(x_t)$
      - $H = \nabla^2f(x_t)$
- Possible issues:
  - $H$ may fail to be positive definite: Can switch to gradient descent (Levenburg-Marquandt)
  - Instead of inverting $H$ directly, can solve $Hy = g$ for $y$ and define $x_{t+1} = x_t - y$
    - can be computationally more efficient
  - Could do $x_{t+1} = x_t - \alpha_ty$, with $\alpha_t > 0$
    - I.e. a learning rate/step size

## 15.3-15.4: Logistic Regression (Binary) Introduction:

- General
  - Does classification
  - Widely used model
  - can give pretty good performance
  - builds the foundation for neural networks, generalized linear models
  - Can do multiclass, but we'll only talk about binary logistic regression here
- Example: Suppose you are are an actuary
  - Want to model the $p(death| x)$, where $x_1$ is age, $x_2$ is M/F, $x_3$ is cholesterol
  - simplest model $w_0 + \sum_i w_ix_i = w^tx$ where $x = (1,x_1,x_2,x_3)$
    - But this is not a probability
    - To get a probability, send through a sigmoid: $p(death|x) = \sigma(w^Tx)$
      - $\sigma(a) = e^a/(e^a + 1)$
- Formalization
  - Classification algorithm
  - Given: $D = \{(x_1,y_1),\dots,(x_n,y_n)\}$, $x_i \in \mathbb{R}^d$, $y_i\in \{0,1\}$
  - Model: $Y_i\sim Bernoulli(\sigma(w^Tx_i))$
    - independent
    - $\sigma(a) = e^a/(1+e^a)$
    - $w$ is a parameter
    - Discriminative model: No Probability distribution on $x$
- Pros
  - Interpretable: if $w_i$ is positive, then the probability increases as $x_i$ grows (and vice-versa)
  - Small number of parameters, $d+1$, i.e. statistically efficient
  - Computationally efficient to estimate $w$ (Newton's method to maximize likelihood)
  - Natural extension to multiclass
  - Foundational method for neural networks, generalized linear models
- Cons
  - Performance can be weaker (e.g. than Random Forests, Boosted Trees, SVMs, etc.)

## 15.5-15.7: Estimating parameters for Binary Logistic Regression

- Maximum likelihood estimation: $w_{MLE} = \arg\max_w p(D|w)$
  - $p(D|w) = \prod_i p(y_i|x_i,w) = \prod_i \alpha_i^{y_i}(1-\alpha_i)^{1-y_i}$, where $\alpha_i = \sigma(w^Tx_i)$
  - we can't solve analytically for $w$ because of the nonlinearity in $\sigma$
  - Let's work with logs
    - Minimize: $\mathcal{L}(w) = -\log p(D|w) = -\sum_i y_i\log \alpha_i + (1-y_i)\log(1-\alpha_i)$
- Gradient, i.e. take a derivative wrt $w_j$
  - let $\alpha = \sigma(w^Tx)$
  - $\frac{d}{dw_j}\log\alpha = \frac{x_je^{-w^Tx}}{1 + e^{-w^Tx}} = \frac{x_j}{e^{w^Tx} + 1} = x_j(1-\alpha)$, note: $\log \alpha = \log\sigma(w^Tx) = \log\frac{1}{1 + e^{-w^Tx}} = -\log(1+e^{-w^Tx})$
  - $\frac{d}{dw_j} \log(1-\alpha) = -x_j + x_j(1-\alpha) = -\alpha x_j$, note: $\log(1-\alpha) = -w^Tx - \log(1 + e^{-w^Tx})$
  - Thus, the derivative of $\mathcal{L}$:
    - Given a data point $x_i$, we denote $x_{ij}$ to be the $j$-th dimension of $x_i$, i.e. $x_i = (x_{i1},\dots x_{id})$
    - $\frac{d}{dw_j}\mathcal{L} = -\sum_i y_ix_{ij}(1-\alpha_i) - (1-y_i)\alpha_ix_{ij} =\sum_i (\alpha_i-y_i)x_{ij}$
      - remember, $y_i$ is in $\{0,1\}$, $\alpha_i$ is the probability from the distribution
    - Let $A$ be the design matrix, where $A_{ij} = x_{ij}$
      - the derivative of the negative log likelihood simplifies to $A^T(\alpha-y)$
    - (Note that if we set this to zero the $\alpha$ will expand and this becomes very messy)
- Hessian: Take one more derivative
  - $\frac{d^2}{dw_jdw_k}\mathcal{L} = \sum_i x_{ij}\frac{d}{dw_k}\alpha_i$
    - From before, we know the derivative of $\log\alpha$
      - Thus $d\alpha = \alpha d\log\alpha = \alpha x_j(1-\alpha)$
      - i.e. $\frac{d}{w_k}\alpha_i = x_{ik}\alpha_i(1-\alpha_i)$
    - So we obtain $\sum_i x_{ij}x_{ik}\alpha_i(1-\alpha_i) = z_j^TBz_k$, where
      - $z_j = x_{1j},\dots,x_{nj}$
      - $B$ is a diagonal $n\times n$ matrix with diagonal elements $\alpha_i(1-\alpha_i)$
        - Note that the $\alpha$ are outputs of $\sigma$, so that the diagonal elements are positive, real valued
          - We can define $B^{1/2}$ with elements the square roots of those of $B$
    - So the entire Hessian Matrix (all $ij$ in a matrix form) is $H = A^TBA$
      - This is $(B^{1/2}A)^T(B^{1/2}A)$, which is positive semi-definite: Thus $\mathcal{L}$ is convex and we can apply Newton's Method
- Applying Newton's Method
  - In this case, known as "iteratively reweighted least squares"
  - Update rule: $w_{t+1} = w_t - H^{-1}g$
    - analogy to 1D case: $x_{t+1} = x_t - f'(x_t)/f''(x_t)$
    - $w_{t+1} = w_t - H^{-1}g = w_t - (A^TBA)^{-1}A^T(\alpha-y)$
      - assume that $H$ is invertible
    - rewrite as a weighted least squares algorithm
      - $(A^TBA)^{-1}A^TB(Aw_t - B^{-1}(\alpha - y)) = (A^TBA)^{-1}A^TBz_t$
        - where $z_t = Aw_t-B^{-1}(\alpha - y)$
      - in linear regression, the weights were solution was $(A^TA)^{-1}A^Ty$
      - By analogy, this is linear regression where:
        - minimize $\arg\min_w \sum_i b_i(z_i - w^Tx_i)^2$
        - We weight by the elements of $B$; $i$-th element is weighted by $\alpha_i(1-\alpha_i)$
  - Iterative weighted least squares:
    - in each iteration, we look at $z_i - w^Tx_i$
    - $B$ changes in each iteration

# 16: Expectation Maximization

## 16.1 - 16.2: $K$-means clustering

- $K$-means clustering
  - Given $D = (x_1,\dots,x_n)$ in $\mathbb{R}^d$
  - Intuition: Points from a cluster are closer to the "center" of their cluster
  - Assume that there are $K$ clusters with unknown "centers" $\mu_i$
  - A measure $\mathcal{L}$ of how good proposed centers are is the sum of distances, i.e. $\sum_i ||x_i-\mu_j||^2$, where point $i$ is assigned to cluster $j$
    - rewrite as $\mathcal{L} = \sum_{j=1}^K\sum_{i=1}^n a_{ij}||x_i-\mu_j||^2$, where $a_{ij}$ is 1 if $i$ is assigned to $j$ and 0 otherwise
  - Algorithm
    - initialize $\mu_i$ to some values
    - Choose optimal assignments $a$ given centers $\mu_i$: i.e. assign $x_i$ to closest $\mu_j$
    - Choose optimal $\mu$ for fixed $a$: i.e. set to the centroid/empirical mean of the assigned points
      - take the gradient of $\mathcal{L}$ wrt $\mu_j$, set to 0, see that the Hessian is positive definite (if each cluster has at least one point)
    - repeat until convergence
  - not guaranteed to give the global minimum
- $K$-means is a special case of Expectation Maximization

## 16.3-16.5: Expectation Maximization (EM) Algorightm

- Class of Algorithms/approach to approximately obtain MLE or MAP estimates of parameters when some data is missing
  - For example, HMM: hidden states are not observed, Gaussian Mixture Models, etc.
  - Useful when we have unobserved latent variables
- Setup
  - Given: $x = (x_1,\dots,x_n)$
  - Model: $X,Z\sim p_\theta$ for some unknown parameter $\theta\in\Theta$
    - works best when $p_\theta$ is an exponential family
  - Goal: find $\theta_{MLE}\in\arg\max_\theta p_\theta(x)$
  - Don't get to see the hidden variables $Z$, so the marginal distribution $p_\theta(x) = \sum_z p_\theta(x,z)$ is difficult to maximize
- EM Algorithm:
  - Iteratively improve estimate of $\theta$
    - initialize $\theta_0\in\Theta$
    - for $t = 0,1,\dots$ until convergence or max number of iterations
      - $E$-step: $Q(\theta,\theta_t) = \mathbb{E}_{\theta_t}(\log p_\theta (X,Z)|X=x)$
        - ideally as an expression of $\theta$
      - $M$-step: Choose $\theta_{t+1}$ to maximize $Q(\theta,\theta_t)$
- Pros:
  - Guaranteed that $p_{\theta_{t+1}}(x) \geq p_{\theta_t}(x)$
  - Works well in practice
- Cons:
  - Not guaranteed to give $\theta_{MLE}$: can get stuck at a local optimum
  - MLE can overfit
    - but we can also use EM to get the MAP
  - Convergence can be slow
  - Specialized to Exponential Families
    - if not exponential, one of the $E$ or $M$ steps can be difficult to solve analytically
- Motivation for the EM algorithm
  - Consider the MLE case
  - Goal: find $\arg\max_\theta p_\theta(x)$
  - Specialize to an expnential family: $p_\theta(x,z) = e^{g(\theta)^Ts(x,z)}h(x,z)/c(\theta)$
  - For exponential families, MLE is invariant under reparameterization, so we assume $g(\theta)=\theta$
    - We assume we have "natural form" $p_\theta(x,z) = e^{\theta^Ts(x,z)}h(x,z)/c(\theta)$
  - Differentiate and set to zero: can maximize log instead
    - $0 = \frac{d}{d\theta_i} \log p_\theta(x) = \frac{1}{p_\theta(x)}\sum_z\frac{d}{d\theta_i}p_\theta(x,z)$
      - $\frac{d}{d\theta_i}p_\theta(x,z) = (s_i(x,z) - \frac{d}{d\theta_i}\log c(\theta))p_\theta(x,z)$
      - $\frac{d}{d\theta_i}\log c(\theta) = \mathbb{E}_\theta (s_i(X,Z))$
        - the sufficient statistic
    - $0 = \frac{1}{p_\theta(x)}\sum_z (s_i(x,z) - \mathbb{E}_\theta s_i(X,Z))p_\theta(x,z) = \sum_z s_i(x,z)p_\theta(z|x)-\mathbb{E}_\theta s_i(X,Z)$
      - note that $s_i(X,Z)$ doesn't depend on $z$
    - First sum is $\mathbb{E}_\theta(s_i(X,Z)|X=x)$, so we obtain
      - $\mathbb{E}_\theta s_i(X,Z) = \mathbb{E}_\theta(s_i(X,Z)|X=x)$
      - often can't solve this analytically for $\theta$
      - Iterate:
        - Let $\theta_0\in\Theta$
        - for $t = 1,2,\dots$
          - Solve $\mathbb{E}_{\theta_{t-1}}(s_i(X,Z)|X=x) = \mathbb{E}_{\theta_t}s_i(X,Z)$ for $\theta_t$
  - Standard EM: $Q(\theta,\theta_t) = \mathbb{E}_{\theta_t}(\log p_\theta (X,Z)|X=x)$
    - $p_\theta(x,z) = e^{\theta^Ts(x,z)-\log c(\theta)}h(x,z)$
    - $\log p_\theta(x,z) = \theta^Ts(x,z) - \log c(\theta) + \log h(x,z)$
    - $Q(\theta,\theta_t) = \mathbb{E}(\log p_\theta(X,Z)|X=x) = \theta^T\mathbb{E}(s(X,Z)|X=x)-\log c(\theta) + C$
      - To maximize this, we different wrt $\theta_i$, set to zero
        - $0 = \mathbb{E}(s_i(X,Z)|X=x) - \mathbb{E}s_i(X,Z)$
        - i.e. we get the same condition as before

## 16.6: Gaussian mixture model: intro

- mixture of Gaussian distributions
  - pdf is a convex combination of Gaussians
  - can extend to multiple dimensions of course
- Generative model
  - Setup
    - $z \in \{e_1,\dots,e_m\}$
      - $e_i$ form the standard basis for $\mathbb{R}^m$
    - $p(z=e_k) = \alpha_k$
  - Given $z = e_k$, $X \sim N(\mu_k, C_k)$
    - $\mu_1,\dots,\mu_m\in \mathbb{R}^d$
    - $C_1,\dots,c_m$ are $d\times d$ covariance matrices
  - In other words, $z$ is a "latent variable"
    - First draw $z$: i.e. a Gaussian
    - Then draw a normally distributed $x$ from that Gaussian
    - etc.
  - instead of modeling the pdf, we model via: first pick which distribution, then pick a value of $x$ corresponding to that.
    - $p(x) = \sum_z p(x|z)p(z) = \sum_k p(z = e_k)p(x|z = e_k) = \sum_k \alpha_k N(x|\mu_kC_k)$
  - $\alpha_i$ are called the mixing coefficients
- Joint distribution
  - $p(x,z) = \prod_{k=1}^m \alpha_k^{z_k}N(x|\mu_k,C_k)^{z_k}$
    - $z_i$ are indicator functions, ie which distribution we pick
  - $p(z = e_k|x) = \frac{p(x|e_k)p(e_k)}{p(x)} = \frac{\alpha_k N(x|\mu_k,C_k)}{\sum_l \alpha_lN(x|\mu_l,C_l)} = \mathbb{E}(I(z=e_k)|X=x)$

## 16.7-16.10: EM for the Gaussian mixture model


- How to estimate the parameters for a GMM
  - difficult, but pretty nice with EM
- Parameters: $\theta = (\alpha, \{\mu_k\}, \{C_k\})$
- EM: we'll use sufficient statistics for exponential families
  - $\mathbb{E}_{\theta_{t-1}}(s_i(X,Z)|X=x) = \mathbb{E}_{\theta_t}s_i(X,Z)$
  - $\mathbb{E}_{\theta_0}(s_i(X,Z)|X=x) = \mathbb{E}_{\theta}s_i(X,Z)$ to make it easier
- Let's show that it's an exponential family
  - $p(x,z) = \prod_k \alpha_k^{z_k}N(x|\mu_k, C_k)^{z_k} = \prod_k e^{z_k\log \alpha_k}\frac{1}{\sqrt{|2\pi C_k|}}\exp(-\frac{1}{2}(x-\mu_k)^TC^{-1}(x-\mu_k))$
    - define $\lambda_k = C_k^{-1}$, collect exp, put into exp form, apply some properties of the trace
    - sufficient statistics are $z$, $z_kxx^T$ for $k = 1,\dots,m$, $z_kx$ for $k = 1,\dots,m$
  - now we extend do iid draws of $x$ and $z$, i.e. iid draws of $p(x,z)$ - for each sample
    - $p(D) = \prod_i p_\theta(x_i,z_i)$
      - $x_i = (x_{i1},\dots x_{ik})$, $z_i = (z_{i1},\dots,z_{im})$
      - do more arithmetic, read off sufficient statistics, apply the EM algorithm for exponential families
- then, given $\theta_0$, we can calculate $\mathbb{E}_{\theta_0}(\sum_i z_{ik}|X=x)$, and then can calculate $\theta_1$, etc.
  - to calculate $\theta_i$, calculate $\mu$ and the covariance matrix
  - (derivations in the videos gets a bit convoluted)
  - after a bunch of arithmetic, we obtain update functions for $\theta_i$ from $\theta_{i-1}$

## 16.11 - 16.12: The likelihood is nondecreasing under EM

- We want the MLE: $\theta_{MLE} = \arg\max_\theta p_\theta(x)$
  - turns out that EM is not guaranteed to give us this
- But if our data is $x = (x_1,\dots,x_n)$, the sequence $\theta_1,\dots$ of EM estimates satisfies $p_{\theta_i}(x)\leq p_{\theta_{i+1}}(x)$
- Proof:
  - $\theta_{t+1} \in \arg\max_\theta Q(\theta, \theta_t)$ where
    - $Q(\theta, \theta_0) = \mathbb{E}_{\theta_0}(\log p_\theta(X,Z)|X=x)$
      - $\theta_0$ is just a generic $\theta$ (e.g. $\theta_t$ as we go through the algorithm)
    - (E step: compute $Q$, M step: maximize $Q$ given $\theta$)
  - define $l(\theta) = \log p_\theta(x)$
    - $\log$ is monotone, so we can prove the statement for $l$ (the log likelihood) instead
  - Part I: 
    - Suppose that for some function $g(\theta)$, 
      - (i) $l(\theta) \geq Q(\theta, \theta_0) + g(\theta_0)$ for any $\theta, \theta_0$
        - define $h_{\theta_0}(\theta) = Q(\theta, \theta_0) + g(\theta_0)$
      - (ii) $l(\theta) = Q(\theta,\theta) + g(\theta)$ for any $\theta$
    - By (i), $l(\theta)$ is an upper bound for $h_{\theta_0}(\theta)$ for any $\theta_0$
      - (aka fix $\theta_0$ and consider $h_{\theta_0}$ as we change $\theta$ around: the values of $h_{\theta_t}(\theta)$ are always underneath $l(\theta)$)
    - $l(\theta) = h_\theta(\theta)$ by definition. In particular, $l(\theta_t) = h_{\theta_t}(\theta_t)$
    - in other words, the function $h_{\theta_t}$ is always underneath or equal to $l$ and coincides with $l$ at $\theta_t$
    - So in the maximization step, when we maximize $Q(\theta, \theta_t)$ we also maximize $h_{\theta_t}$
      - Then the maximizer $\theta_{t+1}$ of $h_{\theta_t}$ takes a value under $h_{\theta_t}$ that is more than $l(\theta_t)$ and less than $l(\theta_{t+1})$, and we have our result
        - $l(\theta_t) = h_{\theta_t}(\theta_t) \leq h_{\theta_t}(\theta_{t+1}) \leq l(\theta_{t+1})$
        - $l(\theta_t) = Q(\theta_t,\theta_t) + g(\theta_t)\leq \max_\theta Q(\theta,\theta_t) + g(\theta_t) = Q(\theta_{t+1}, \theta_t) + g(\theta_t) \leq l(\theta_{t+1})$
  - Part II:
    - $l(\theta) = \log p_\theta(x) = \sum_z q(z)\log p_\theta(x)$
      - for any pmf $q$
    - $l(\theta) = \sum_z q(z)\log\big(p_\theta(x)p_\theta(z|x)\frac{q(z)}{p_\theta(z|x)}\frac{1}{q(z)}\big) = \sum_zq(z)\log p_\theta(x,z) + \sum_z q(z)\log\frac{q(z)}{p_\theta(z|x)} -\sum_zq(z)\log(q(z))$
      - $-\sum_z q(z)\log q(z) = H(q)$, the entropy of $q$
      - $\sum_z q(z)\log\frac{q(z)}{p_\theta(z|x)} = D(q||p)$, the relative entropy of $q$ and $p$
      - (we make some definitions of what happens in corner cases: $0\log 0 = 0$, middle term is $0$ if $p_\theta(z|x) = 0$)
    - now choose $q(z) = p_{\theta_0}(z|x) = \sum_z p_{\theta_0}(z|x) + \log p_{\theta_0}(x,z) + D(p_{\theta_0},p_{\theta}) + H(p_{\theta_0}(\cdot|z)) = \mathbb{E}_{\theta_0}(\log p_\theta(X,Z)|X=x) + g(\theta_0) = Q(\theta, \theta_0) + g(\theta_0)$
      - i.e. we define $g(\theta_0)$ as the $D+H$ remainder
      - check that the conditions are satisfied
        - the first one is satisfied because it was how we ended up with our definition of $g$
        - if $\theta = \theta_0$, then $D(p_{\theta_0},p_\theta) = 0$ and we obtain (ii)

## 16.13: EM for MAP estimation

- so far, EM only for MLE estimation
  - but it's always better to do a MAP as MLE is prone to overfitting
- Assume a prior $p(\theta)$, data $x = (x_1,\dots,x_n)$, latent variables $z$
- Goal: find $\theta_{MAP} \in \arg\max_\theta p(\theta|x)$
- Algorithm
  - Initialize $\theta_0\in \Theta$
  - for $t = 1,2,\dots$
    - E-step: $R(\theta, \theta_{t-1}) = \mathbb{E}_{\theta_{t-1}}(\log p(X,Z|\theta)|X=x) + \log p(\theta)$
      - $\log p(\theta)$ is the log of the prior of $\theta$
    - M-step: take $\theta_t \in \arg\max_\theta R(\theta, \theta_{t-1})$
- Can do it analytically for exponential families etc.
- Claim: the likelihoods $p_\theta(x)$ are non-decreasing
- Proof:
  - $\mathcal{L}(\theta) = \log p(x, \theta)$
    - we have a prior now for $\theta$, so we can consider the joint distribution
  - $\mathcal{L}(\theta) = \log p(x|\theta) + \log p(\theta) = l(\theta) + \log p(\theta)$
    - $= Q(\theta, \theta_0) + D(p(\cdot|x, \theta_0),p(\cdot|x,\theta)) + H(p(\cdot|x,\theta_0)) + \log p(\theta)$
      - let $g(\theta_0) = H(p(\cdot|x,\theta_0))$
      - then as for MLE, this $\geq R(\theta,\theta_0) + g(\theta_0)$ with equality when $\theta = \theta_0$ and we obtain $\mathcal{L}(\theta_t)\leq \mathcal{L}(\theta_{t+1})$
- of course, nondecreasing doesn't even guarantee that we hit a local max, let alone a global max

# 17: Sampling

## 17.1: Sampling methods - why, pros, cons

- Why sampling?
  - can approximate expectations
    - estimate statistics
    - posterior inference
  - visualization of "typical draws" of a distribution
- why expectations?
  - any probability is an expectation
    - $P(X\in A) = \mathbb{E} I(X\in A)$, i.e. the expected value of an indicator
  - often need approximations for sums/integrals, and many intractable sums/integrals can be written as expectations
- Pros
  - Easy to implement and understand compared to exact inference or deterministic approximations
  - General purpose: can be used in many situations
- Cons
  - It's too easy - often used inappropriately
  - Slower than deterministic/exact methods
  - Getting "good" samples may be difficult
  - Difficult to assess the performance of the method

## 17.2 - 17.4: Monte Carlo methods

- History
  - Idea is trivial, but formal mathematics starting in the 1940s
    - Enrico Fermi: Did Monte Carlo approximations by hand, was often close
    - John Von Neumann: worked on computers
    - Fermi, Von Neumann, and Stan Ulam were in Los Alamos in the 40s for the atomic bomb
    - 1946: Ulam used an approximate method to simulate card deals
      - thought of using Von Neumann's computers and Fermi's method to solve problems
      - and then they did it
  - really works once computers are invented
  - Name: Stan Ulam had an uncle who often went to Monte Carlo
- The method
  - Goal: Approximate an expectation $\mathbb{E} f(X)$ of a real-valued function $f(X)$
    - intractable to compute analytically
  - Definition: If $X_1,\dots,X_n\sim p$ iid, then $\hat{\mu}_n = (1/n)\sum_i f(X_i)$ is a (basic) Monte Carlo estimator of $\mathbb{E} f(X)$ where $X\sim p$
    - approximate the mean via the sample mean
  - Remarks
    - $\mathbb{E} \hat{\mu}_n = \mathbb{E} f(X)$, i.e. an unbiased estimator
    - $\hat{\mu}_n$ converges to $\mathbb{E} f(X)$ as $n$ goes to $\infty$
      - i.e. for all $\epsilon > 0$, $P(|\hat{\mu}_n-\mathbb{E}f(X)| < \epsilon)$ goes to 1
      - aka it's a "consistent estimator
      - assuming that $\sigma^2(f(X)) < \infty$, i.e. finite variance
    - $\sigma^2(\hat{\mu}_n) = (1/n)\sigma^2(f(X))$
      - because variance of a sum of independent random variables is the sum of the variances
      - also assuming that $\sigma^2(f(X)) < \infty$, i.e. finite variance
      - i.e. converges to 0 as $n$ goes to infinity. "at a rate of $1/\sqrt{n}$"
        - regardless of the dimension of $X$
    - remember that $MSE = bias^2 + variance^2$
      - so MSE also goes to zero as $n$ goes to $\infty$
  - Additional note: even though we have the convergence, calculating $\sigma^2(f(X))$ can be vry non-trivial
- Examples of Monte Carlo
  - Approximate $\pi$: sample from square, count the points in the circle vs out of the circle
  - Area of Mandelbrot Set
    - Contained in the square $S = [-2,2]\times [-2,2]$
    - defined as the set of complex numbers $c$ such that the sequence $\{z_n\}$ is bounded, where $z_1 = 0$, $z_{n+1} = z_n^2 + c$
    - Monte Carlo: uniformly take random elements of $S$, see if they fall in the set, etc.
  - Expected return of an investment strategy
    - e.g. return over 30 days: $T=30$
    - model as a random process $X = (X_1,\dots, X_T)$
    - suppose we have a strategy $S$ with return $R_S(X)$
    - Monte Carlo to find the expected return: do a bunch of possibilities

## 17.5 - 17.7: Importance Sampling

- not actually sampling: an approximation of Monte Carlo methods
- Monte Carlo: approximate $\mathbb{E} f(X) \approx \frac{1}{n}\sum_i f(X_i)$
  - with $X\sim p$ and $X_i \sim p$
  - but sometimes, the assumption that $X_i$ is also drawn from the same distribution is not true
- Importance Sampling: can correct for drawing from a different distribution $q$ than $p$, and can in fact do "better"
  - Assume $p$ is a density
  - $\mathbb{E} f(X) = \int f(x)p(x)dx \approx f(x)\frac{p(x)}{q(x)} dx$
    - valid for all pdf $q$ such that $q(x) = 0$ implies that $p(x) = 0$
    - now we can do Monte Carlo on $f(x)\frac{p(x)}{q(x)}$, i.e. by $\frac{1}{n}\sum_i f(X_i)\frac{p(X_i)}{q(X_i)}$
  - i.e. $\mathbb{E} f(X)\sim \frac{1}{n}\sum_i f(X_i)w(X_i)$, where $w(x) = p(x)/q(x)$ and $X_i \sim q$
    - $w$'s are called "importance weights"
  - as this is the Monte Carlo of $f(x)\frac{p(x)}{q(x)}$, we also have that it is unbiased, variance goes to 0, MSE goes to 0
    - if we make a clever choice of $q$, we can get them to go to $0$ faster
      - $\sigma^2(I_n) = \frac{1}{n}\sigma^2\big(f(x)\frac{p(x)}{q(x)}\big)$
- Intuition
  - Suppose that we invest with some strategy, and we get returns $f(x)$ that are terrible for some range of $x$ and only moderately positive otherwise
    - suppose that these terrible $x$ results very rarely, so that they get very small $p$
      - then with Monte Carlo, it'll take us a long time to realize that they exist
        - very rare to get a terrible sample
      - with importance sampling, we can bias our sampling to the terrible events
        - and more quickly get these terrible samples
  - Suppose $f$ is very highly positive for some values of $x$, and $p$ has most of it's mass for these values of $x$ - and pretty small otherwise
    - but we have difficulty sampling with $q$ from the areas where $f$ is small
      - then Importance Sampling will do poorly here
  - moral: choose $q$ to be large where $|f(x)|p(x)$ is large
    - but it may be hard to know which situation we're in
- without normalization constants
  - had assumed that
    - can sample efficiently from $q$
    - can efficiently evaluate $f$ and $w$ at these points
  - but these assumptions can be broken: we often don't know $p$ or $q$
  - Suppose that we know $p$ and $q$ up to a normalizing constant
    - $p(x) = \tilde{p}(x)/z_p$, $q(x) = \tilde{q}/z_q$
      - but we don't know $z_p$ or $z_q$
    - $\mathbb{E} f(X) = \int f(x) p(x) = \int f(x)\frac{p(x)}{q(x)}q(x) = \int f(x)\frac{\tilde{p}(x)}{\tilde{q}(x)}\frac{z_q}{z_p}q(x)$
    - now sample from $q$ via Monte Carlo
      - $\approx \frac{z_q}{z_p}\frac{1}{n}\sum_i f(X_i)\tilde{w}(X_i)$
        - $\tilde{w}(x) = \tilde{p}(x)/\tilde{q}(x)$
        - $\frac{z_p}{z_q} = \frac{1}{z_q}\int \tilde{p}(x) = \int \frac{\tilde{p}(x)}{\tilde{q}(x)}q(x)\approx \frac{1}{n}\sum_i \tilde{w}(X_i)$ (approx is via Monte Carlo)
        - Plug in, simplify: $\mathbb{E} f(X)\approx \frac{1}{n}\sum_i \hat{w}(X_i)f(X_i)$ where 
          - $X_i \sim q$
          - $\hat{w}(X_i) = \frac{\tilde{w}(X_i)}{(1/n)\sum_i \tilde{w}(X_i)}$
          - $\tilde{w}(x) = \tilde{p}(x)/\tilde{q}(x)$

## 17.8 - 17.10: Smirnov transform (Inverse transform sampling)

- a way of generating random samples from a pdf on the real numbers
  - e.g. have real valued random variable, can generate random samples from it
- intution: invert the CDF of a random variable
  - Suppose that $X\sim N(0,1)$
    - consider the CDF $F(x) = P(X \leq x)$
    - to sample from $X$, we randomly draw from $[0,1]$ and consider $F^{-1}$ of that number
- Invertible case: Let $F:\mathbb{R} \rightarrow (0,1)$ be an invertible CDF
  - invertible: $\exists F^{-1}:(0,1)\rightarrow \mathbb{R}$ s.t. $F^{-1}(F(x)) = x$ for all $x\in \mathbb{R}$, $F(F^{-1}(a)) = a$ for all $a\in (0,1)$
  - if $U\sim Uniform(0,1)$, then $F^{-1}(U)\sim F$, i.e. $F^{-1}(U)$ has CDF $F$.
  - Pf: Let $x,y\in \mathbb{R}$, $a,b\in (0,1)$
    - remarks:
      - $x\leq y$ implies $F(x)\leq F(y)$
      - $x < y$ implies $F(x) < F(y)$: $F$ is invertible, so $F(x) \neq F(y)$ if $x\neq y$
      - $a \leq b$ implies $F^{-1}(a)\leq F^{-1}(b)$: if $F^{-1}(a) > F^{-1}(b)$, then $a = F(F^{-1}(a)) > F(F^{-1}(b)) = b$, i.e. a contradiction
      - $F^{-1}(a)\leq x$ if and only if $a\leq F(x)$: combine previous conclusions
      - The CDF of the uniform distribution $H$ is $0$ if $x\leq 0$, $x$ for $x\in (0,1)$ and $1$ for $x\geq 1$
    - $P(F^{-1}(U)\leq x) = P(U\leq F(x)) = H(F(x)) = F(x)$
      - thus $F$ is the cdf of the random variable $F^{-1}(U)$
  - this doesn't work for more general cases (e.g. discrete case: $F$ is not invertible)
- General Case
  - often non-invertible: e.g. exponential distribution, discrete
  - Setup:
    - $F:\mathbb{R}\rightarrow [0,1]$ a CDF 
    - $G:(0,1)\rightarrow \mathbb{R}$ by $G(a) = \inf \{x\in \mathbb{R}: a \leq F(x)\}$
  - Statement: if $U\sim Uniform(0,1)$ then $G(U)$ has CDF $F$
  - Remarks
    - If $F:\mathbb{R} \rightarrow (0,1)$ is invertible then $G = F^{-1}$
    - $G(a) \in (0,1)$ as $F(x) \rightarrow \infty$ as $x$ goes to $\infty$ and $F(x)\rightarrow 0$ as $x$ goes to $-\infty$
  - Proof:
    - Claim: $\forall a\in (0,1)$ and $\forall y\in \mathbb{R}$, then $a\leq F(y)$ iff $G(a)\leq y$
      - Proof:
        - if $a\leq F(y)$, then $G(a) = \inf \{x| a\leq F(x)\}$. $y$ is in this set by assumption, so by the definition of $y$, then $G(a)\leq y$
        - if $G(a)\leq y$. As $G(a)$ goes to the extremes of $(0,1)$ (see remark), then the set in $G(a)$ is non-empty. Let $x_1,x_2,\dots \geq G(a)$ be a sequence of elements that converge to $G(a)$. Then $a\leq F(x_n)$ for all $n$. So if we take the limit, $a\leq\lim F(x_n) = F(G(a)) \leq F(y)$
    - Proof: same as for the invertible case
- Example: Sampling an exponential random variable
  - $X\sim Exp(\lambda)$, $f(x) = \lambda e^{-\lambda x}$ for $x \geq 0$ and $0$ otherwise
  - Then the CDF is $F(x) = 1 - e^{-\lambda x}$ for $x \geq 0$ and 0 otherwise
    - CDF is not invertible, but is invertible for $x > 0$
    - apply the generalized case, define $G(a) = \inf (x\in \mathbb{R} | a \leq F(x)$ for $a\in (0,1))$
      - $a = 1 - e^{-\lambda x}$ thus $x = (-1/\lambda)\log (1-a)$
        - for an $a$ in $(0,1)$, this is the $x$ that we sample
  - so we select $a$ uniformly, and these are the points that we get that are distributed exponentially as desired
- Note: doesn't really work for Gaussians for example, as we can't get a closed form solution for the inverse like we did here

## 17.11 - 17.14: Rejection Sampling

- Info
  - Simple and clever way to draw random samples from complicated pdfs.
  - very general, can be higher dimensional spaces
  - can be hard to do efficiently in large dimensions
- Uniform case
  - Goal: Generate samples from the uniform distribution on a set $A$, where $A$ is a complicated set.
  - Intuition: draw a bunch of samples from a larger set, reject samples that are not in our set.
  - Example: Sample $X$ uniformly over the Mandelbrot set
  - Prop: If $A\subset B$ and $Y_1,Y_2,\dots \sim Unif(B)$ iid, and $X= Y_K$ where $K = \min \{k: Y_k\in A\}$, then $X\sim Unif(A)$
    - Pf: Exercise (Hint - do the discrete case first using Geometric series)
- Non-Uniform case
  - Intuition:
    - Suppose we have a pdf, and another pdf that is nonzero where the first is nonzero
    - To select from the first distribution: select from the second distribution, randomly reject based off of the ratio of the two distributions
  - Goal: Sample from a complicated pdf on $\mathbb{R}^d$.
  - Given: $\tilde{f}$, where $f(x) = \tilde{f}(x)/\alpha$ where $/alpha > 0$
    - $\tilde{f}$ can be easier to deal with/evaluate than $f$
  - Algorithm:
    - Choose a "proposal" distribution $q$ such that $\exists c > 0$ with $cq(x)\geq \tilde{f}(x)$ for all $x$
      - we want $q$ to be easy to sample from.
    - Draw a sample $X\sim q$, then sample $Y\sim Unif(0, cq(X))$ given $X$
    - If $y \leq \tilde{f}(X)$, then set $z=x$. Otherwise, "reject" this sample and return to the previous step.
    - the output is $z$, which is distributed according to $f$
  - Intuition:
    - $\tilde{f}$ is proportional to the pdf $f$
    - $q$ is a pdf that we can scale up such that it contains $\tilde{f}$
    - Now we sample $x$ from q, and then sample uniformly from 0 to the scaled up version of $q$ at $x$
      - if we're under $\tilde{f}$, then take $x$ as a sample for $f$
      - otherwise, draw a new $x'$ and then sample uniformly for this new $x'$, etc.
    - if we select an $x$ where $cq$ is much higher than $\tilde{f}$, then we're almost certain to reject.
    - but if we select an $x$ where $cq$ is relatively close to $\tilde{f}$, then we're much more likely to accept
  - Proof of algorithm
    - A: Prove that the second step selects points uniformly from the points under the curve $cq$
    - B: In the final step, we reject points that are outside of $\tilde{f}$ - and end up uniformly drawing points from the points below $\tilde{f}$.
    - C: then looking at the $x$ points (i.e. project down to the axis), we've drawn uniformly from $f$
    - Lemma: If $X\sim q$, $Y|X\sim Unif(0,cq(X))$ then the the pair $(X,Y)$ is uniformly distributed on $B$, where $B = \{(x,y):x\in \mathbb{R}^d, 0 < y < cq(x)\}$
      - I.e. A
      - Pf
        - If $y\notin (0,cq(x))$, then $p(x,y) = p(y|x)p(x) = 0$ as $p(y|x)= 0$
        - Otherwise, $p(x,y) = p(y|x)p(x) = (1/(cq(x))) q(x) = 1/c$, i.e. uniform.
    - B is obvious
    - Lemma: If $(X,Y) \sim Unif(A)$, where $A = \{(x,y)|x\in \mathbb{R}^d, 0 < y < \tilde{f}(x)\}$, then $X\sim f$
      - I.e. C
      - Pf: The volume $m(A) = \int \tilde{f}(x)dx = \int \alpha f(x)dx = \alpha$
        - More explicitly, $\int_A b dxdy = b\int_x \int_0^{\tilde{f}(x)} dyy dx = b\int_x \tilde{f}(x) = b \alpha$, i.e. $b = 1/\alpha$
        - $p(x) = \int_\mathbb{R} p(x,y)dy = \int \frac{1}{\alpha}I(0<y<\tilde{f}(x)) dy = \tilde{f}(x)/\alpha = f(x)$
    - Prop: Suppose $f$ and $q$ are pdfs on $\mathbb{R}^d$ such that $\tilde{f} = \alpha f(x)$ for $\alpha > 0$ and $cq(x)\geq \tilde{f}(x)$ for all $x\in \mathbb{R}^d$ with $c > 0$. Thenif $X_1, X_2, \dots \sim q$ and $Y_k|X_k \sim Unif (0, cq(X_k))$ and $Z = X_k$ where $K = \min \{k: Y_k\leq \tilde{f}(X_k)\}$. Then $Z\sim f$
    - Pf: put together the pieces we proved earlier
  - a good $q$ is easy to draw fraom and can fit quite tightly to $\tilde{f}$, as the space between $cq$ and $\tilde{f}$ is essentially wasted space
