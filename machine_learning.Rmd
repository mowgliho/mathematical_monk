...
- If $X~Beta(a,b)$, then $\mathbb{E} X = a/(a+b)$, $\sigma^2(X) = ab/((a+b)^2(a+b+1))$, mode (maximum density) is $(a-1)/(a+b-2)$
...
- for $\theta = (\theta_1,\dots,\theta_m) = \theta ~ Dir(\alpha)$ if $p(\theta) = \frac{1}{B(\alpha)}\prod_{i=1}^m \theta_i^{\alpha_i -1} I(\theta\in S)$
