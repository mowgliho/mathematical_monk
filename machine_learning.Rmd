...
- Let's classify a new point given $x$:
  ...
  - $\prod_{i=1}^n p(x^i,y^i|\theta) = \prod_i Cat(y^i|\pi)\prod_{jk}Cat(x^i_j|r_{jk})^{I(y_i=k)}$
  - $p(\theta) = Dir(\pi|\alpha)\prod_{j,k}Dir(r_{jk}|\beta)$
  - Thus, $p(y|x,D) \propto \int_\theta p(x,y|\theta) Dir(\pi|\alpha + c)\prod_{jk}Dir(r_{jk}|\beta + d_{jk}) d\theta$
    ...
    - $p(x,y|\theta) = \pi(y)\prod_{j,k}r_{jk}(x_j)^{I(y=k)}$
  - grouping stuff, we have
