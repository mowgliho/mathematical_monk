...
## 10.1-10.2: Posterior for linear regression
...
- Posterior: $p(w|D)\propto p(D|w)p(w) \propto \exp \big(-\frac{a}{2}(y-Aw)^T(y-Aw)-\frac{b}{2}w^Tw\big)$
  ...
  - matching with the (note) below, we define
    - $\Lambda = aA^TA + bI$
    - $\mu = a\Lambda^{-1}A^Ty$
  ...
  - the MAP estimate is then $\mu = a\Lambda^{-1}A^Ty = (A^TA + \frac{b}{a}I)^{-1}A^Ty$
    ...
    - the difference is $b/aI$ in the inverse. $b/aI$ is known as the "regularization parameter" related to $-b/2 w^Tw$
